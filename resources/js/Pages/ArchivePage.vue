<template>
    <Content-box class="h-fit p-7 flex flex-col space-y-3">
        <div class="flex flex-row pt-2 space-x-10 justify-between  pb-2">
            <div class="flex flex-row space-x-10 justify-start w-3/4">
                <!-- <v-select v-model="province" class="w-1/5" :options="provinces" label="province" placeholder="Provinces"/>
                <v-select v-model="municipality" class="w-1/5" :options="municipalities" label="municipality" placeholder="Municipality"/> -->
                <Datepicker v-model="date" show-now-button class="w-1/5" :enable-time-picker="false" aria-placeholder="Date" />
            </div>
            ...
            <tbody>
                <tr class="max-h-full even:bg-orange-200  bg-white border-b dark:bg-gray-800 dark:border-gray-700"
                    ...
                    <td class="text-center">
                        {{ moment(body.INC_submitted_at).format('MMMM DD Y') }}
                    </td>
                    ...
                    <td class="text-center gap-">
                        ...
                        <a type="button" class=" font-medium text-red-600  dark:text-orange-500 hover:underline">
                            <!-- <EditModal :form="body" /> -->
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                stroke="currentColor" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round"
